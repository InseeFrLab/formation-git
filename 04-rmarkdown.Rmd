...
Un `R Markdown`, à la différence d'un `Markdown` classique, ne fait pas que présenter un code `R`, il l'exécute également. Cela donne, par exemple, le rendu suivant:
...
### La philosophie `R markdown` ? 


R Markdown fonctionne : 

* différemment d’`Office`. C’est un éditeur qui n'est pas *WYSIWYG* (*What you see is what you get*) : le fichier est composé de texte brut et, éventuellement, de code informatique. C'est une fois le fichier compilé que la mise en forme peut être visualisée dans le fichier de sortie. 
* en dissociant le fond de la forme. On parle de *WYSIWYW* (*What you see is what you want*): 
...
Les connaisseurs du format `Latex` reconnaîtront une grande similarité dans la philosophie avec `Markdown`. En effet, dans les deux cas, la mise en forme est appelée par des commandes spéciales. On ne voit pas le texte mis en forme italique dans le document de texte mais dans le résultat une fois compilé. 


## Pourquoi utiliser `R markdown`, quels sont ses avantages ? 


Dissociation entre le fond de la forme, ce qui:

    * allège les mises à jour ou correctifs
    * donne de la visibilité au code ayant généré les résultats et illustrations

Possibilité de commenter l'ensemble du document, ce qui:
...
* pour commenter : 
    * Dans le texte, pour activer/désactiver un commentaire c'est facile (balises de commentaire équivalentes à celle du format `html`) : **ctl + Maj + c**
    * ![](./pics/04_markdown/Commentaire.png)
    * à l'intérieur des blocs de code `R`, les commentaires se font comme dans un script `R` (avec le commentaire classique en `R`: `#`)
...
Actuellement, il n'existe pas d'outils disponible pour intégrer automatiquement la charte graphique Insee dans un document `Markdown`. Des thèmes `ggplot` et des feuilles de style (*template*) adaptées aux différents formats de publication Insee pourraient permettre la construction de publications à partir de `Rmarkdown` qui seraient plus faciles à faire évoluer au grès des relectures et plus reproductibles pour les études qu succèdent. 
...
Tout comme pour un script R, les ressources (images, données, ...)


```

Tout comme pour un script R, les ressources (images, données, ...) doivent être appelées via des chemins d'accès relatifs au dossier du projet.

Donc pas de chemin complet.
...
1. **En-tête** contenant les métadonnées du document. Elle est délimitée entre deux lignes de pointillés. On trouve parfois le nom de `YAML header` pour désigner le nom de cette partie.
...
![](./pics/04_markdown/inserer_img.png)
...
1. Insertion par délimiteur **`** (`alt Gr + 7`): pour insérer le résultat d'une instruction simple, à l'intérieur d'un texte, on utilise la syntaxe suivante: 

![](./pics/04_markdown/inserer Rsimple.png)
...
2. Insertion d'un bloc de code R : les `chunks`
...
![](./pics/04_markdown/inserer_R.png)
...
* **Remarque importante** : à chaque compilation une nouvelle session R est lancée, vierge de tout objet créé. Les premiers blocs de code sont alors souvent dédiés au chargement des packages, à l'importation des données, à la définition de l'environnement et au recodage.
...
Exemple avec le jeu de données Insee `histoires de vie 2003 (hdv2003)` inclus directement dans la package **questionr**. Avant de commencer, on va utiliser le `pipe` du package `magrittr`, plutôt que d'attacher le package (cf. Section \@ref(functions), on va uniquement importer la fonction `%>%` grâce au package `import`:
...
```{r, echo=TRUE}
knitr::kable(res, caption = "hdv2003 - bricol x cuisine - afichage avec la fonction kable")
```
...
![](./pics/04_markdown/cheat sheet Rmarkdown.png)

Il existe un éditeur de `markdown` (pas de `R markdown`) sur la plateforme innovation: [`CodiMD`](https://hackmd.beta.innovation.insee.eu). Par rapport à une approche via `Rstudio`, l'avantage de `CodiMD`, permet de visualiser en direct le résultat
