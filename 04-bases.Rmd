...
+ `r with_def("commit")` : je valide mes modifications avec un message qui les explique
...
Dans le cadre de cette formation, un projet d'exemple a été créé pour vous. L'idée est de former des groupes de deux, chaque groupe représentant une équipe. Vous allez donc travailler en binôme avec votre voisin (ou avec une personne de la visio si la formation n'a pas lieu en présentiel). Vous avez besoin de désigner une personne référente (disons X pour la suite de l'énoncé).
...
de 2, X doit faire un `r with_def("fork")` qui revient à dupliquer le projet pour chacune des équipes. Pour cela, il faut cliquer en haut à droite du projet exemple, qui sert de base à l'exercice.

Après avoir initié le `fork`, X doit ensuite donner des droits à son/sa binôme (dans `members` du menu à gauche). Accorder le droit de `maintainer` pour commencer (une erreur de type 403 "access denied" se produirait plus tard sinon).
...
`r emo::ji("warning")` N'oubliez pas qu'il faut avoir fait un `fork` du projet (un seul suffit, par la personne X désignée).
...
Cloner le dossier de votre projet d'équipe avec `RStudio` via le lien `SSH` du projet sur `Gitlab`. Ensuite, X doit modifier le fichier `.gitignore` en y ajoutant les fichiers `.pdf` et au `.html` ainsi que `.Rproj`. X doit ensuite sauvegarder et pusher le `.gitignore`. Les autres personnes du groupe peuvent ensuite récupérer ces modifications via `pull`.
...
4. Ouvrir le menu `commit` de `RStudio` et faites `Revert` puis répondez oui à la boîte de dialogue qui s'ouvre (après l'avoir lue et comprise). Et vous retrouvez vos deux fichiers.
...
3. Pour cela, aller dans l'historique des commits et identifier le `SHA` du commit responsable. Taper dans le `GIT bash` (ouvert via RStudio ou bien en vérifiant qu'on est dans le bon dossier) :
