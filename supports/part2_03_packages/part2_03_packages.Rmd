---
title: "Créer un package `R` par la pratique"
subtitle: "Pas à pas"
author: "Romain Lesur"
...
if (system.file(package = "klippy") == '') 
  remotes::install_github("rlesur/klippy", upgrade = "never")
```
...
Certes, la création de packages nécessite d'acquérir de nouvelles connaissances et notions mais elles ne sont pas particulièrement complexes. De plus, ces notions sont largement transposables d'un langage de programmation à un autre car la plupart des langages utilisent la notion de packages (parfois appelés librairies ou modules dans d'autres langages). 

Un avantage de la création de packages est que cela va permettre de structurer vos programmes et d'y ajouter beaucoup de rigueur. La principale compétence à laquelle on fera donc appel lors de la réalisation d'un package est **la rigueur**. Cette rigueur réduit le coût pour se réapproprier un code fait il y a quelques temps ou fait par un autre. C'est donc une pratique à recommander dans le cadre d'un travail collaboratif. 

Dès qu'on a un doute sur la conception d'un package, on peut se référer au livre d'[Hadley](http://r-pkgs.had.co.nz/) sur le sujet

## Quand créer un package ? {-}

Voici une règle d'or qui devrait vous guider pour savoir quand vous **devez** créer un package:

- lorsque vous réalisez une analyse, si vous utilisez plus de deux fois une même portion de code (que vous avez "copié-collé"), vous devez créer une fonction. Le *copier-coller* est une pratique risquée car on risque toujours d'introduire une source d'erreur lors de ce geste manuel.
...
**Créer un package doit donc devenir une habitude de travail et une bonne pratique**.
...
Un package offre des fonctions prêtes à l'emploi. Ces fonctions sont disponibles après un appel à `library()` ou bien en utilisant `::`. Ces fonctions "prêtes à l'emploi" sont des fonctions dites **exportées** par le package.

Cette notion est à distinguer des fonctions **internes** à un package auxquelles l'utilisateur ne peut accéder.^[il existe une technique pour accéder aux fonctions internes d'un package mais il n'est jamais recommandé de l'utiliser. Nous ne la présenterons donc pas.]

C'est le créateur du package qui décide des fonctions qu'il souhaite exporter ou bien conserver pour une utilisation interne sachant que l'utilisateur extérieur ne pourra accéder qu'aux fonctions exportées
...
Il s'agit de l'action que vous réaliserez le plus fréquemment. Si vous négligiez de le faire, vous pourriez à terme vous retrouver dans des situations très délicates.

Il y a 3 façons de réaliser ce contrôle de conformité :
...
```{r, eval=FALSE, code=readLines("cheatsheet.R", encoding="UTF-8")}
```
...
Le document détaille chaque étape de la feuille de route. Vous pouvez le lire dans l'ordre successif ou aller directement à la partie qui vous intéresse. 
...
C'est sans doute l'étape la plus délicate. **Il faut trouver un nouveau nom de package qui est disponible**.\
En effet, si vous aviez l'idée de créer un nouveau package avec le nom `dplyr` et que vous l'installiez, vous écraseriez le package `dplyr` que vous utilisez régulièrement.
...
Les principales fonctions du package **available** sont `available_on_cran()`, `available_on_bioc()` et `available_on_github()`.

Par exemple, pour vérifier si le nom de package `dplyr` est disponible, exécutez :
...
Bien évidemment, il faut posséder l'équivalent du couple identifiant/mot de passe pour pouvoir effectuer une telle modification sur votre profil. Cet équivalent s'appelle un jeton d'accès privé (_private access token_).
...
Si vous n'avez pas initialisé de `README` lors de la création du dépôt (cf. plus haut) `gitlab` vous propose les lignes de commande nécessaires que vous pouvez copier-coller. Une fois la connection au dépôt distant effectuée, par exemple via `usethis::use_git_remote(url = repo_url)`, vous pouvez utiliser le bouton `push ` de l'onglet `git` de `Rstudio`. 
...
Cette commande crée le fichier `R/fix_missing.R` et l'ouvre.

Il ne vous reste plus qu'à inclure le code source de cette fonction.

Comme nous l'avons vu précédemment, nous devons documenter cette fonction.

Pour cela, nous pouvons inclure un squelette de documentation grâce à la commande RStudio _Code > Insert Roxygen Skeleton_.
...
Il y a aussi la possibilité d'utiliser le raccourci `CTRL+SHIFT+D` ou dans l'onglet `Build` de `Rstudio` de cliquer sur `More` et sélectionner `document`
...
Qu'est ce que le fichier `NAMESPACE` ?

Voici le fichier `NAMESPACE` que nous avons à ce stade :
...
Pour mieux organiser votre travail, vous pouvez bien évidemment créer des fonctions à seul but interne. Dans ce cas, retirez la directive `@export`. Ces fonctions pourront être utilisées seulement à l'intérieur de votre package. Cela peut permettre d'inclure des fonctions couteaux-suisse dans votre package qui effectuent des tâches intermédiaires et n'ont donc pas besoin d'être utilisées par un utilisateur final. 
...
Il existe deux **règles d'or**. La première est que **toute fonctionnalité non testée comprend un bug**. Cette règle a beau être connue par tous, tout le monde a un biais de surconfiance qui le pousse à ne pas tout tester. Et "comme par hasard", les programmes plantent toujours là où on n'a pas implémenté de tests. Tout le monde se fait avoir, **même les développeurs les plus aguerris**. Au cas où vous hésiteriez à écrire des tests, vous voilà prévenus.
...
Enfin, le dernier intérêt des tests est d'éviter que votre package se brise si vous faites des petites modifications ultérieurement (si, si, ça arrive...). Dans des projets de production, où les fonctions s'enchaînent, cette pratique est vitale pour réduire les risques de *bug* non identifiés ou résultats non contrôlés. 

Mais vous allez voir, écrire des tests est quelque chose de simple et amusant. Nous allons utiliser le package **testthat** pour nous aider à les écrire et générer de beaux rapports. Les tests ont vocation à tourner sur des petits jeux de données d'exemple à la structure équivalente aux données utilisées dans le projet: il ne s'agit pas de faire tourner un test sur les DADS ! 

Nous allons écrire des tests pour la fonction `fix_missing()` de notre package. Pour créer un nouveau fichier qui comprendra nos tests, nous allons utiliser la fonction `usethis::use_test()`. Exécutez la commande :
...
L'ensemble des fonctions (`test_that()`, `expect_*()`...) sont issues du package **testthat**. Le principe d'un test est de proposer de manière littéraire le résultat anticipé (par exemple "la multiplication fonctionne") et effectuer un test formel (test d'égalité, logique...)
...
_La suite de cette histoire est imaginaire mais arrive souvent dans la réalité. En effet, nous allons voir qu'il y a un méchant bug de caché..._
...
Essayez de le faire par vous-même. Pour la documentation de la fonction, vous pouvez utiliser l'astuce suivante : il est possible de réutiliser la documentation d'un paramètre d'une fonction grâce à la directive `@inheritParams nomdelafonction`^[pour réutiliser la documentation d'un autre package, utilisez `@inheritParams package::nomdelafonction`]. Dans notre exemple, cela peut être utile pour documenter le paramètre `na.value` de la fonction `fix_missing_df` en réutilisant celle de la fonction `fix_missing`. Vous évitez ainsi de faire des copier-coller et votre documentation sera plus facile à mettre à jour. **Utilisez cette astuce le plus souvent possible**, notamment dans le cas des fonctions emboîtées. Pour le paramètre `...`, la directive est `@inheritDotParams`.
...
Une fois un package déclaré dans la rubrique `Imports` du fichier `DESCRIPTION`, vous devez donc utiliser `::` pour utiliser ses fonctions dans votre code source. Cependant, il y a des cas où l'utilisation de `::` n'est vraiment pas aisée et peut dégrader la lisibilité de votre code.
...
N'oubliez pas de relancer le contrôle de conformité (`devtools::check()`) pour vérifier que vous avez bien résolu le warning. Puis effectuer un _commit_ suivi d'un _psuh_.

Dès lors, la fonction `map()` du package **purrr** pourra être utilisée telle quelle dans le code source de votre package.
...
Le fichier `README` est le premier fichier qui s'affiche lorsque vous consultez votre projet sous GitLab. Tous les projets doivent disposer d'un fichier `README` : cela fait partie du B.A.-BA.  
Ne pas disposer d'un tel fichier est considéré comme un manque de professionnalisme important. **Vous devez donc le créer**.  
...
S'il est nécessaire d'introduire du code `R` à exécuter dans le fichier `README`, il est possible d'utiliser R Markdown grâce à la commande : 
...
Comme vous avez pu le constater, un package c'est du code R avec beaucoup de documentation autour ainsi que des tests.

L'intérêt de regrouper votre travail dans un package est que cela vous obligera à documenter au fur et à mesure (sinon le contrôle de conformité sera en erreur), ce qui est moins pénible que de tout documenter à la fin d'un projet.
