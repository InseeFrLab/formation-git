...
L'authentification est alors automatique depuis le poste de travail Insee via le login. Il en est de même sur la plateforme innovation via le login `Insee SSO`, si tant est qu'un profil utilisateur a été préalablement créé. Il suffira alors de cliquer à chaque pour être connecté en tant qu'utilisateur et accéder à son profil :
...
Il est généralement plus simple d'utiliser l'authentification `HTTPS` que `SSH`,
notamment lorsqu'on interagit avec `Gitlab.com` car il suffit
de rentrer `login/password` lorsqu'on interagit avec un dépôt.

Pour sécuriser son compte, il est recommandé de ne pas utiliser son mot de passe à chaque push/pull (c'est d'ailleurs interdit sur Github depuis quelques temps) mais de créer un jeton d'accès (*token*) qui est un mot de passe aléatoire, qui peut être révoqué à tout moment, mais bénéficiant de moins de droits qu'un mot de passe habituel (seules certaines opérations sont autorisées avec ce mot de passe). La fiche [utilitR](https://www.book.utilitr.org/git-config.html) présente avec plus de détails ces concepts et des recommandations de sécurité adaptées, par exemple sur la manière d'enregistrer ce
jeton pour ne pas avoir à le fournir à chaque authentification. 

Si vous désirez tout de même utiliser l'authentification SSH, la
fiche [utilitR](https://www.book.utilitr.org/git-config.html) déjà citées
présente la démarche. 
