...
Les chargés d'étude ont une copie locale du projet sur leur poste de travail. Les changements sont mis à jour et disponible en temps réel. Il existe plusieurs manières d'utiliser le contrôle de version: 

* en ligne de commande, via [git bash](https://gitforwindows.org/) ;
* avec une interface graphique spécialisée, par exemple [tortoise git](https://tortoisegit.org/)
...
Ensuite, les deux fonctions `push` et `pull` permettent la mise à jour du projet local par rapport au distant et réciproquement. Lors de ces étapes des **conflits** peuvent exister, c'est par exemple le cas lors que deux personnes ont modifié le même programme simultanément. 
...
* Mettre à jour sa copie locale régulièrement via `pull` ou `fetch`, voire dès qu'on commencer à travailler sur un projet.


:::: {.rmdimportant data-latex=""}

De manière générale, **il ne faut pas avoir peur des conflits**. Les outils sont faits pour les gérer avec rigueur et méthode (voir EXERCICE). Avec GIT, il est très rare de faire des bêtises et on peut toujours réparer. Mais il n'est justement pas recommandé d'utiliser les commandes de la forme `git push --force`. 
...
2. `CI/CD`: espace dédié à l'intégration continue (cf. Section avancée ** sur le sujet)
2. `Wiki`: espace de documentation collaboratif séparé des fichiers du dépôt ([exemple](https://github.com/Rdatatable/data.table/wiki)). Cela implique que la documentation n'est pas associée à un fichier du dépôt. 
...
L'affichage est similaire à celui de [Trello](https://trello.com/) mais a l'avantage d'évoluer de manière automatique au grès des ouvertures et fermetures *d'issues*. 


### Merge requests

Espace pour gérer des fusions de branches. Normalement, on a arrive dans un deuxième temps dans cet espace:
...
* `Delete source branch when merge request is accepted`: effacer la MR sur le dépôt commun pour éviter que quelqu'un l'édite alors qu'elle n'est plus d'actualité. 
* `Squash commits when merge request is accepted`: Regrouper tous les `commit` (potentiellement très nombreux) en un seul dans la branche de destination. Cela évite, sur les gros projets, des branches avec des milliers de *commits*.
...
Les gros projets *open-source* fonctionnent plutôt de la première manière. 


### Les groupes


Quand un projet est important, on peut créer un groupe qui permet d'avoir une arborescence de fichiers:
...
Cette formation adopte le parti pris de suivre le `Github flow` donc une rapide description peut être trouvée sur [cette page](https://guides.github.com/introduction/flow/).
...
**Question a.** *Création d'une branche*

projet commun, branche individuelle

**Question b.** *Modification au sein d'une branche*

deux prhrases sans ponctuation sur votre branche

**Question c.** *Changer de branche*

changer pour votre voisin, corriger et revenir à votre branche initiale
...
créer une MR proprement avec GitLab (indications à ajouter)

**Question b.** *Contribuer à une MR*

Choisissez une autre MR et contribuez-y via GitLab.

**Question c.** *Régler une MR*

Fusionner la branche de votre voisin en réglant la MR.


Usuellement, c'est la seule responsabilité des mainteneurs : seuls eux peuvent fusionner dans `master`. Il leur appartient de veiller à la bonne qualité des `merge requests`.
