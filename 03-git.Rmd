...
La documentation [utilitR](www.book.utilitr.org) constitue une ressource
...
:::: {.exo data-latex=""}
Exercice 3: J'ai fait une bêtise ? Ouf, j'utilise Git
...
4. Ouvrir un `terminal` (plus de détails dans la Section XXX) et taper

```shell
git remote --hard HEAD
```

vous retrouvez vos deux fichiers.
...
La seconde brique afin de pouvoir travailler sur un projet partagé est le choix de la plateforme où est stocké le `repository`. Cela peut être sur `github.com`, `gitlab.com` ou une autre forge en ligne. À l'Insee, il est recommandé de s'appuyer sur le [`GitLab` interne](https://git.stable.innovation.insee.eu/). Pour des utilisations plus avancées, il également possible d'utiliser la [plateforme innovation](https://onyxia.beta.innovation.insee.eu/accueil). 
...
Le `GitLab` est accessible via ce lien : [gitlab.insee.fr](https://gitlab.insee.fr).  Pour accéder à son profil sur le  `GitLab` de l'Insee, il est nécessaire de se connecter avec son profil individuel. Pour y accéder, il suffit de cliquer sur `Sign In` en haut à droite: 

![](./pics/03_git/sign_in_gitlab.png){width=100%} 

L'authentification est alors automatique depuis le poste de travail Insee via le login. Il en est de même sur la plateforme innovation via le login `Insee SSO`, si tant est qu'un profil utilisateur a été préalablemen créé. Il suffira alors de cliquer à chaque pour être connecté en tant qu'utilisateur et accéder à son profil :
...
::: {.remarque}
Pour débuter, il est généralement plus simple d'utiliser l'authentification `HTTPS` que `SSH`, notamment lorsqu'on interagit avec `Gitlab.com` car il suffit de rentrer `login/password` lorsqu'on interagit avec un dépôt. Cependant, l'usage du `Gitlab` interne à l'Insee nécessite de configurer une clé SSH.

La section suivant présente ainsi les étapes nécessaires pour configurer une clé SSH à l'Insee pour pouvoir utiliser `GIT` sur AUSv3 notamment.
:::
...
:::{#AUS .tabcontent}
#### Sur AUSv3 {-}
La marche à suivre, pas à pas, est décrite sous AUSv3, dans le fichier `Y:/Documentation/AUSV3/Git_Utiliser git sous AUSv3.pdf`
:::
::: {#local .tabcontent}
#### En local {-}
...
#### Enregistrer une clé SSH dans `Gitlab` {#configSSHgitlab}

Dans le cas d'une configuration d'une projet vie SSH, après avoir créé la clef, il est nécessaire de retourner sur les paramètres du profil sur `GitLab` et accéder aux réglages de l'espace personnel : 
...
Les étapes précédentes étaient des configurations à ne faire qu'une seule fois.
